        ...
        <ul className="space-y-2">
          {Array.isArray(lessons) && lessons.map(l => (
            <li
              ...
              className={`cursor-pointer p-2 rounded ${l.id == lessonId ? 'bg-blue-200' : 'hover:bg-gray-200'}`}
            >
              {l.title} {l.completed ? '✅' : ''}
            </li>
          ))}
      ...
      <main className="flex-1 p-6">
        ...
        <div className="flex items-center space-x-4 mt-6">
          <button
            ...
            className={`px-4 py-2 rounded ${lesson.completed ? 'bg-gray-400' : 'bg-green-600 hover:bg-green-700 text-white'}`}
          >
